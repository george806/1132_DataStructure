...
private:
    ...
    TreeNode* deleteNode(TreeNode* node, int val) {
    	if (val < node->data) { //如果要刪的值比目前這個節點小,往左子樹找
		 ...
		 if (node->left == nullptr && node->right == nullptr) {//沒有子節點 
            delete node;
            return nullptr;
        }
        else if (node->left == nullptr) {//只有右子節點，其節點會遞補 
            TreeNode* temp = node->right;
            delete node;
            return temp;
        }
        else if (node->right == nullptr) {//只有左子節點，其節點會遞補 
            TreeNode* temp = node->left;
            delete node;
            return temp;
        }
        else {//左右都有子節點
            TreeNode* ss= findMin(node->right); // 從右邊遞補的方法，找右子樹最小值 
            node->data = ss->data;               // 用它取代目前節點的值
            node->right = deleteNode(node->right, ss->data); // 把它刪掉，避免出現兩個相同的 ，然後接回去 
        }
		}
    }
