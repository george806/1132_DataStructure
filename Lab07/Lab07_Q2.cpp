...
public:
    ...
    void inorderTraversal(TreeNode* node) {
		...
		if(swi)
			maxL=node->value;//指定左方最大值 
		else
			maxR=node->value;//指定右方最大值 
        inorderTraversal(node->left);  // 遍歷左子樹
        cout << node->value << " ";    // 訪問當前節點
        if(node->value==root->value) swi=false;//確認root位置，不放第一是因為一進去就會遇到 
...
int main() {
    ...
    cout<< endl;
    cout<<"最大左子數值: "<<maxL;//輸出最大左數值 
	cout<< endl; 
    cout<<"最大右子數值: "<<maxR; //輸出最大右數值 
